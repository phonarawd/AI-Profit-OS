...
`services/api-nest` + `schemas` 는 protected-scope root 이며 현재 REL-502가 NOT_ISSUED / REBASE_REQUIRED 상태다.
따라서 기능 alignment는 유지하지만 R7 인증 권위는 `CERT_ISSUED=0 · STALE_PENDING_REBASE=1`로 mirror한다. REL-502 current-epoch 재발급 전 ISSUED 세탁 금지.
...
| migration_head | local `20260903092000` | remote applied `20260902155632` | DIVERGE · source-only hardening pending | REL-701-DB |
| p0_p3_engine | REL-502 `FINAL_ACCEPTANCE` | DEFECTS_P0/P1 = 0 | FUNCTIONALLY CLEAN · certification STALE_PENDING_REBASE | REL-502 |
...
| protected_scope | live hash | baseline | DRIFT=25 · certification pending rebase | REL-502 |

## 3. VERIFY

| command | result |
|---|---|
| `pnpm verify:backend-data-alignment` | live table · current-fx wired · Engine issuance mirrored fail-closed |
| `pnpm verify:rel-505-r7-backend-alignment` | this document |
| `pnpm verify:rel-508-current-fx-approx` | Nest wire |

## EXIT_GATE

- REL-502가 REBASE_REQUIRED이면 R7은 CERT_ISSUED=0 · STALE_PENDING_REBASE=1이어야 하며, current-epoch ISSUED 후에만 반대로 전환
