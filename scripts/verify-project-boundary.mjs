#!/usr/bin/env node
/**
 * Fixture-only project boundary verify — runtime Cursor hooks OFF.
 * Policy library + hook script still tested. Never opens foreign project contents.
 */
...
// --- 20–25 architecture: Cursor runtime hooks OFF (lowspec · PO) ---
const hooksJson = JSON.parse(
  fs.readFileSync(HOOKS_JSON_PATH, "utf8").replace(/^\uFEFF/, "")
);
const eventNames = Object.keys(hooksJson.hooks || {}).sort();
const wiredCount = eventNames.reduce(
  (n, ev) => n + ((hooksJson.hooks[ev] || []).length),
  0
);
expect("20 no beforeReadFile", !hooksJson.hooks.beforeReadFile);
expect("21 no beforeShellExecution", !hooksJson.hooks.beforeShellExecution);
expect("22 no beforeMCPExecution", !hooksJson.hooks.beforeMCPExecution);
...
expect(
  "runtime Cursor hooks unwired",
  wiredCount === 0 && eventNames.length === 0,
  eventNames.join(",")
);
expect(
  "no preToolUse wiring",
  !hooksJson.hooks.preToolUse || hooksJson.hooks.preToolUse.length === 0
);
expect(
  "no beforeTabFileRead wiring",
  !hooksJson.hooks.beforeTabFileRead ||
    hooksJson.hooks.beforeTabFileRead.length === 0
);
...
expect("25 hooks.json wires zero runtime scripts", wiredScripts.length === 0);
...
const report = {
  ...
  RUNTIME_HOOKS: "OFF",
  residual_WARNs: [
    "Cursor runtime hooks are unwired (lowspec). Isolation is agent-rule + policy fixture only.",
    "Desktop multi-root adding a foreign folder is a USER-CONFIGURATION residual.",
  ],
  cases,
