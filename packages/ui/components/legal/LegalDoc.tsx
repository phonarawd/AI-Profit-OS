...
export function LegalDoc({
  ...
  return (
    ...
    >
      ...
      <p className="mt-3 text-sm leading-relaxed text-pd-text-muted">{intro}</p>
      {sections?.map((s) => (
        <section key={s.title} className="mt-5">
          <h2 className="text-sm font-semibold">{s.title}</h2>
          <p className="mt-2 text-sm leading-relaxed text-pd-text-muted">
            {s.body}
          </p>
        </section>
      ))}
      {showTax ? (
        <p className="mt-6 text-xs text-pd-text-muted">{T.legal.taxDisclaimer}</p>
      ) : null}
