...
export function DemoWalletBanner({
  ...
  return (
    <aside
      ...
      className="mt-4 rounded-[var(--radius-md)] border border-[var(--color-lux-border)] bg-[var(--color-lux-elevated)] px-4 py-3"
      role="status"
    >
      <div className="flex items-center gap-2">
        <span
          data-testid="practice-badge"
          className="rounded px-2 py-0.5 text-xs font-semibold text-[var(--color-lux-accent)]"
        >
          {T.practice.badge}
        </span>
        <p className="text-sm font-medium text-[var(--color-lux-text)]">
      ...
      {practiceUsdt !== undefined && !zero ? (
        <p
          className="mt-2 text-sm text-[var(--color-lux-text)]"
