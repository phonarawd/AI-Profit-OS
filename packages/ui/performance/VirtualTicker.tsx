...
export function VirtualTicker({
  ...
  if (rows.length <= threshold) {
    return (
      <ul
        className={[
          "flex max-h-24 flex-col gap-1 overflow-y-auto text-sm text-pd-text-muted",
          className,
        ]
          .filter(Boolean)
          .join(" ")}
      ...
      >
        {rows.map((row) => (
          <li key={row.id} data-testid="ticker-row" className="truncate pd-motion-any">
            {row.text}
          </li>
        ))}
      </ul>
  ...
  return (
    <VirtualList
      ...
      overscan={3}
      height={height}
      className={["text-sm text-pd-text-muted", className]
        .filter(Boolean)
        .join(" ")}
      testId="virtual-ticker"
      renderItem={(row) => (
        <div data-testid="ticker-row" className="truncate pd-motion-any leading-7">
          {row.text}
        </div>
      )}
    />
