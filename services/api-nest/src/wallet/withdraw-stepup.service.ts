...
export class WithdrawStepUpService {
  ...
  assertStepUpToken(input: {
    ...
    const expect = this.macToken(userId!, method!, challengeId!, expiresAtSec);
  ...
  /**
   * Consume a valid step-up token exactly once at a protected money boundary.
   * Cryptographic validation remains non-mutating so an already-created
   * idempotent intent can be replayed safely; only a new protected action
   * reaches this atomic spend marker.
   */
