...
import { allowFourEyeTxId } from "./four-eye-allowlist";
...
export class UsdtWithdrawBroadcastService {
  ...
  async drainApprovedWithdrawals(opts?: {
    ...
    for (const row of rows.rows) {
      ...
      try {
        ...
        this.log.warn(`withdraw broadcast fail id=${row.id}: ${String(err)}`);
        skipped += 1;
      }
    }
  ...
  /**
   * Queue a KMS-signed TRC20 transfer. Private key never leaves local KMS.
   * Tatum returns signatureId pending → KMS daemon signs → tx hash.
   */
  async queueTrc20Transfer(opts: {
    ...
    if (!res.ok) {
      // Do not log body (may contain sensitive upstream detail)
    ...
    // Four-eye: allow KMS daemon to sign this pending id (localhost only).
    if (!json.txId) {
      allowFourEyeTxId(txId);
    }
    return {
      txId,
