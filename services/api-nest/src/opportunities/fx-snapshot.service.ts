...
/** Consumer current-FX apply producer. Marketplace legs omitted. */
...
export class FxSnapshotService {
  ...
  /**
   * Additive current-FX read for Consumer apply.
   * Reuses private loadLatest(). Does not reinterpret getLatestUsableSnapshot.
   */
  async getLatestCurrentFxSnapshot(): Promise<CurrentFxSnapshot | null> {
    if (!this.db.configured()) return null;
    const row = await this.loadLatest();
    if (!row) return null;
    if (!row.usd_krw || !isPositiveAmount(row.usd_krw)) return null;
    return {
      fxSnapshotId: row.id,
      usdtKrw: row.usd_krw,
