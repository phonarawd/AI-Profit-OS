 ...
 * ADAPTER_INGEST_TOKEN is mandatory. Missing configuration fails closed before ingest.
 */
@Controller()
export class AdaptersIngestController {
  ...
  ingest(
    ...
    if (!token) {
      throw new ServiceUnavailableException("ADAPTER_INGEST_TOKEN_UNAVAILABLE");
    }
