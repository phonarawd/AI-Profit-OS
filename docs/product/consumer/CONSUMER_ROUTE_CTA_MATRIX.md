...
`/profits/[id]` participate/preflight는 실배선. `/trades`·execute는 아직 `PendingFigma`. Home·`/profits` 목록 presentation은 discovery only(목록 POST 0). 아래는 **아키텍처 매핑**이다. 계약=`CONSUMER_CORE_LOOP_CONTRACT.md`.
