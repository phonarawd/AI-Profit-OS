#!/usr/bin/env node
/**
 * 운영 Nest(Render)만 재배포한다.
 * Cloudflare web/ops · REL-701 artifact · 실돈 스위치와 분리.
 */
"use strict";
...
async function main() {
  ...
  if (!/^[0-9a-f]{40}$/i.test(sha)) {
    throw new Error("commit SHA required — default branch deploy forbidden");
  }
  if (sha.toLowerCase() === FORBIDDEN_SHA) {
    throw new Error("refused: 7c6a2b0a is not a candidate SHA");
  }
