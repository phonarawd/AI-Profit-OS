...
function run() {
...

  {
    ...
    // product-rebase epochs fail this pin (approved length = 7 after
    // ENGINE_ACCEPTANCE_REBASE_V1 / ea-rebase-395861b0e7aa-a1fe8f018d0f).
    ...
    check(
      ...
      liveBaseline.id === "ea-baseline-395861b0e7aa-a1fe8f018d0f",
      liveBaseline.id,
    );
    // qa9-result is the current-epoch verdict SSOT and is only ever written by
    // run-qa9.cjs. After ea-rebase-395861b0e7aa-a1fe8f018d0f the current-epoch
    // QA1-QA8 rerun + QA9 aggregation issued ENGINE_ACCEPTED_FOR_UI on this tip.
    check(
      "live_verdict_unchanged",
      qa9.verdict === "ENGINE_ACCEPTED_FOR_UI" &&
        qa9.engine_accepted_for_ui === "ISSUED" &&
        qa9.baseline_id === "ea-baseline-395861b0e7aa-a1fe8f018d0f",
      `${qa9.verdict}/${qa9.baseline_id}`,
    );
