...
function harnessOutDir() {
  return process.env.AIPO_QA_HARNESS_OUT || path.join(ROOT, "_tmp_qa_harness", "qa4-clock");
}
 ...
 */
function probeQa4ClockHarness(opts = {}) {
  const dir = opts.dir || harnessOutDir();
  const file = path.join(dir, "qa4-clock-harness.v1.json");
  ...
  if (data.schema !== "harness.qa4-clock.v1") {
    return { available: false, reason: "harness_evidence_schema_mismatch", probed_path };
  }
  if (data.non_canonical !== true || data.does_not_replace_qa4_result !== true) {
  ...
  if (data.security_gate && data.security_gate.ok !== true) {
    return {
      available: false,
