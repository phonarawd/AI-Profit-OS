...
const RULES = [
  ...
  {
    test: (f) =>
      /^apps\/admin\/app\/admin\/execution-policy\//.test(f) ||
      /^tooling\/verify\/rel-209-admin-execution-policy\.cjs$/.test(f),
    scripts: ["match-strictness.cjs", "no-success-rate-percent.cjs"],
  },
  {
    test: (f) =>
