/** Consumer brand = 퍼뜩; ban retired names on backend surfaces */
const fs = require("fs");
...
const manifestRel = "governance/brand/brand.manifest.json";
const manifest = path.join(root, manifestRel);
if (!fs.existsSync(manifest)) {
  console.error("[verify:brand-consumer] FAIL missing " + manifestRel);
  process.exit(1);
}
...
const skip = new Set([manifestRel.replace(/\\/g, "/")]);
const scanDirs = ["services", "schemas"].map((d) => path.join(root, d));
const hits = [];
...
function walk(dir) {
  ...
  for (const ent of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    for (const r of RETIRED) {
      if (t.includes(r)) hits.push(rel + " (::" + r + ")");
    }
  }
}
