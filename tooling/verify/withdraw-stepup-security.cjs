...
assert.doesNotMatch(service, /token_consumed_at/);
assert.match(service, /SET expires_at = to_timestamp\(0\)/);
assert.match(service, /expires_at = to_timestamp\(\$2\)/);
assert.match(service, /expires_at > clock_timestamp\(\)/);
