...
function fixtureCompositePair() {
  const row = cases.find((item) => item.id === "A-composite-owner-vs-derived");
  if (!row) throw new Error("V2_FIXTURE_A_MISSING");
  return row;
}

function isEndedEbay(result) {
  if (!result || result.ok) return false;
  return result.reason === "item_ended" || result.sourceStatus === "UNAVAILABLE";
}
...
async function phaseWriteSo() {
  ...
  const ebay = creds.configured
    ...
    : { ok: false, reason: "EBAY_CREDENTIALS_MISSING" };

  let leftObs = tcg.ok ? tcg.observation : null;
  let rightObs = ebay.ok ? ebay.observation : null;
  let soSource = "LIVE_PAIR";
  if (!leftObs || !rightObs) {
    if (!isEndedEbay(ebay) && ebay.ok === false && ebay.reason !== "EBAY_CREDENTIALS_MISSING") {
      await soRepo.end();
      writeSidecar(sidecarPath, { ok: false, reason: "EBAY_OBSERVE_BLOCKED" });
      process.exit(2);
    }
    const fixture = fixtureCompositePair();
    leftObs = fixture.left;
    rightObs = fixture.right;
    soSource = "V2_FIXTURE_A_AFTER_ITEM_ENDED";
  }

  const tcgWrite = await soRepo.appendObservation(leftObs);
  const ebayWrite = await soRepo.appendObservation(rightObs);
  const ok =
    tcgWrite.ok &&
    ebayWrite.ok &&
