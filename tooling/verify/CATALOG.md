...
| `services/api-nest/src/opportunities/opportunity-write.cjs` · `track-a-opportunity-persist.cjs` · `track-a-opportunity.mapper.cjs` · `tooling/verify/opportunity-write-through.cjs` | opportunity-write-through (T0 isolated PG · Nest write owner · production INSERT 0) |
...
| rel-300-fact-only · rel-301-tool-call · rel-302-safe-refuse · rel-303-red-team · rel-304-grounding · rel-305-bounded-memory | REL-300~305 — existing Coach owners + committed eval fixtures executed (P UNAVAILABLE · G tools=[] · S refuse+over-refusal · red-team · source/asOf+stale · bounded memory/secret redact) — **live** |
| rel-215-admin-ai-logs | REL-215 — `/admin/ai-logs` existing `ai_logs` owner · honest empty · `sanitizeTurnText` · user JWT 401 — **live** |
| rel-400-control-plane-spec | REL-400 — Admin Control Plane superset spec only · 3-mode terms · implementation mix-in 0 — **live** |
| rel-213-admin-system-control | REL-213 — `/admin/system-control` push/reserve/circuit-read · REL-406 catalog honest unavailable · preview→confirm · user JWT 401 — **live** |
| rel-214-admin-audit | REL-214 — `/admin/audit` domain audit panels · REL-405 unavailable · delete UI 0 · AI log mix 0 — **live** |
| rel-401-security-headers | REL-401 — CSP/HSTS/X-Frame-Options/nosniff · shared SSOT · wildcard CSP 0 — **live** |
