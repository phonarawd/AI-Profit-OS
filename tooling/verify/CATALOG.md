...
| 경로 패턴 | verify |
|-----------|--------|
| `docs/product/consumer/**` · `docs/product/PUTDUK_PRODUCT_DESIGN_ENGINEERING_OPERATING_SYSTEM.md` · `tooling/verify/consumer-ux-architecture.cjs` · `tooling/verify/core-loop-contract.cjs` · `tooling/verify/participate-web-wire.cjs` · `tooling/verify/execute-web-wire.cjs` · `governance/consumer-loop/**` · `apps/web/app/profits/[id]/**` · `apps/web/app/trades/**` · `packages/sdk/src/index.ts` · `packages/sdk/src/participate/**` · `packages/sdk/src/execution-stream/**` | consumer-ux-architecture · core-loop-contract · participate-web-wire · execute-web-wire (B-EXECUTION-001 `/trades/[id]/execute` → `useTradeExecution` · /trades 목록은 다음 슬라이스) |
...
| participate-web-wire | Track B B-PARTICIPATION-001 — **live** (`@aipo/sdk/participate` issuePreflight+postParticipate · `/profits/[id]` 실연결 · amountUsdt=requiredCapitalUsdt · 목록 POST 0 · /trades 목록은 B-TRADES-001) |
| execute-web-wire | Track B B-EXECUTION-001 — **live** (`useTradeExecution` · `/trades/[id]/execute` 실데이터 · cookie session tick · consumer 상태 테이블 · progressPct 표시 0 · 레거시 Canon 복구 0 · /trades PendingFigma 유지) |
