...
| 경로 패턴 | verify |
|-----------|--------|
| `docs/product/consumer/**` · `docs/product/PUTDUK_PRODUCT_DESIGN_ENGINEERING_OPERATING_SYSTEM.md` · `tooling/verify/consumer-ux-architecture.cjs` · `tooling/verify/core-loop-contract.cjs` · `tooling/verify/wallet-contract.cjs` · `tooling/verify/wallet-gap-wire.cjs` · `tooling/verify/wallet-release.cjs` · `tooling/verify/lib/wallet-release-runtime.cjs` · `tooling/verify/participate-web-wire.cjs` · `tooling/verify/execute-web-wire.cjs` · `tooling/verify/trades-web-wire.cjs` · `tooling/verify/core-loop-release.cjs` · `tooling/verify/lib/core-loop-release-runtime.cjs` · `tooling/verify/acquisition-contract.cjs` · `tooling/verify/acquisition-gap-wire.cjs` · `tooling/verify/acquisition-release.cjs` · `tooling/verify/lib/acquisition-release-runtime.cjs` · `tooling/verify/account-hub-contract.cjs` · `tooling/verify/account-hub-gap-wire.cjs` · `tooling/verify/account-hub-release.cjs` · `tooling/verify/lib/account-hub-release-runtime.cjs` · `governance/consumer-loop/**` · `governance/consumer-wallet/**` · `governance/consumer-acquisition/**` · `governance/consumer-account-hub/**` · `apps/web/app/profits/[id]/**` · `apps/web/app/trades/**` · `packages/sdk/src/index.ts` · `packages/sdk/src/participate/**` · `packages/sdk/src/execution-stream/**` · `packages/sdk/src/trades/**` | consumer-ux-architecture · core-loop-contract · wallet-contract · wallet-gap-wire · wallet-release · participate-web-wire · execute-web-wire · trades-web-wire · core-loop-release (B-LOOP-002 성공/Safe-Stop E2E) · acquisition-contract (C-ACQ-001) · acquisition-gap-wire (C-ACQ-002) · acquisition-release (C-ACQ-003 guest/auth/error/resume) · account-hub-contract (C-ACC-001) · account-hub-gap-wire (C-ACC-002) · account-hub-release (C-ACC-003 8영역 route-contract) |
| `apps/web/app/auth/**` · `apps/web/app/onboarding/**` · `apps/web/app/ads/**` · `apps/web/app/l/**` · `packages/sdk/src/auth/**` | acquisition-contract · acquisition-gap-wire · acquisition-release (C-ACQ-003) |
| `apps/web/app/me/**` · `governance/consumer-account-hub/**` · `packages/sdk/src/referral/**` · `packages/sdk/src/inbox/**` | account-hub-contract (C-ACC-001) · account-hub-gap-wire (C-ACC-002) · account-hub-release (C-ACC-003) |
...
| `governance/release-master/R7_BACKEND_ALIGNMENT.md` · `governance/release-master/REL-508-MIGRATION-HEAD-IDENTITY.md` · `tooling/verify/rel-505-backend-alignment.cjs` · `tooling/verify/fixtures/r7-backend-alignment.v1.json` · `tooling/verify/fixtures/migrations-remote-applied.v1.json` | rel-505-backend-alignment (REL-505 R7 · C-MIG/C-FSM/C-REASON RECONCILED · HOLDS_OWNED · CLEAN 0) |
...
| `apps/web/app/wallet/page.tsx` · `packages/sdk/src/wallet/**` | wallet-live-wire · wallet-gap-wire · wallet-release |
...
| `services/market-intelligence/src/multi-source-opportunity/**` · `governance/global-product/multi-source-opportunity.v1.json` · `tooling/verify/multi-source-opportunity.cjs` · `services/api-nest/src/opportunities/multi-source-opportunity.contract.ts` | multi-source-opportunity (T0 in-process · EXECUTABLE 교차 소스 Opportunity row 발행 · EXECUTABLE≠Opportunity · CanonicalProduct≠assetId · production INSERT 0) |
...
| home-state-truth | Engine v7.23 R1 — **live** (`schemas/home-read-model.v1.json` · `GET /api/v1/me/home-read` · Money+opportunity+growth+session mapper · todayPossible=server_derived affordable∧available∧compareReady · ledgerTotal=settlement COUNT · viewState · domainFsm null · PART9 home-read binding 보존 · 프레젠테이션 CSS 동결 아님) |
...
| responsive | UI audit §45 · PART8c — Playwright multi-viewport harness (historical). **Owner-approved Visual Master + LOCK 예외** (`visual-master-intake.mdc` · `visual-locks` as current visual authority) = **retired 2026-08-18**. VISUAL_TRUTH=APPROVED_FIGMA_ONLY. T1 `stubs/run-all` 미포함. local=Node structure · `RESPONSIVE_PW=1`=browsers — **retired from T1** |
...
| rel-505-backend-alignment | REL-505 R7 — API·SDK·AppModule·FSM·migration head·RLS/idempotency·auth·money units·source/asOf/reasonCode 1:1 · C-MIG/C-FSM/C-REASON RECONCILED · HOLDS_OWNED (H-TRACK-A / H-DEP-502 / H-DEP-504) · CLEAN 인용 0 · `migrations-applied.v1.json` ≠ remote 1:1 · remote proof=`migrations-remote-applied.v1.json` · path-trigger — **live** |
...
| wallet-contract | Track B B-WALLET-001 — **live** (4레일 Product/Visual/Implementation 계약 · `CONSUMER_WALLET_CONTRACT.md` + `governance/consumer-wallet/wallet.v1.json` · web 8면 PendingFigma 재실측 · Spark Dash DNA=CONSTRAINT_ONLY · Home geometry 종속 0 · Money Rule 재정의 0 · SDK 갭=`wallet-gap-wire`) |
| acquisition-contract | Track C C-ACQ-001 — **live** (Landing/Auth/Signup/Onboarding Product/Visual/Implementation 계약 · `CONSUMER_ACQUISITION_CONTRACT.md` + `governance/consumer-acquisition/acquisition.v1.json` · web 7면 PendingFigma 유지 · Kakao start+exchange PRESENT · SDK auth PRESENT · WEB_GAP_WIRED · Home geometry 종속 0 · Auth Rule 재정의 0 · 배선=`acquisition-gap-wire`) |
| acquisition-gap-wire | Track C C-ACQ-002 — **live** (`@aipo/sdk/auth` session/Kakao start/profile · login/signup/complete-profile/onboarding/ads/l 실배선 · PendingFigma 7 유지 · GuestChrome 최소 래퍼 · 잠금 CTA · 맵/성별/Home geometry 0 · 인증=`acquisition-release`) |
| acquisition-release | Track C C-ACQ-003 — **live** (`verify:acquisition-release` · guest/auth/error/resume 인프로세스 E2E · guest 401 유지 · ads→Core redirect 0 · 기존 Kakao terms 재수집 0 · 신규 TERMS_REQUIRED→signup · oauth deny→login · Stage resume · known defect 0 · LIVE_KAKAO_HUMAN_E2E=NOT_RUN · Auth Rule 재정의 0 · web PendingFigma=7 유지) |
| account-hub-contract | Track C C-ACC-001 — **live** (Profile/Referral/Notifications/KYC/Settings/Support/Guides/Legal Product/Visual/Implementation 계약 · `CONSUMER_ACCOUNT_HUB_CONTRACT.md` + `governance/consumer-account-hub/account-hub.v1.json` · web primary 18 PendingFigma · 호환 4면 보존 · SDK KYC/session/referral/inbox PRESENT · WEB_GAP_WIRED · Home geometry 종속 0 · Auth/Money Rule 재정의 0 · 배선=`account-hub-gap-wire` · 인증=`account-hub-release`) |
| account-hub-gap-wire | Track C C-ACC-002 — **live** (`@aipo/sdk/referral` · `@aipo/sdk/inbox` · logout/delete/disputes · `/me` 8영역 실배선 · PendingFigma 18 유지 · 호환 4면 보존 · 성별/주민번호/%/L1 0 · Home geometry 0 · 인증=`account-hub-release`) |
| account-hub-release | Track C C-ACC-003 — **live** (`verify:account-hub-release` · 핵심 8영역 route-contract 100% · 호환 Membership/Benefits/Events/Strategies 재확인·삭제 0·primary 아님 · known defect 0 · KYC 출금만 · 성별/주민번호/%/L1 0 · Home geometry 0 · Auth/Money Rule 재정의 0 · web PendingFigma 18+4 유지 · HTTP/browser=NOT_THIS_SLICE) |
| wallet-gap-wire | Track B B-WALLET-002 — **live** (deposit-address/KYC/journal SDK+user journal read projection · missing≠0 · 4 rails unchanged · `/wallet/deposit` mixed KEEP · profit→principal CTA HIDE · auth/cross-user negative proof · web pixel 0) |
| wallet-release | Track B B-WALLET-003 — **live** (`verify:wallet-release` · 4레일 money/security 인프로세스 E2E · 1conf≠credit · 19conf 1회 · KRW pending/matched≠credit · approve 1회 · default mode=profit · KYC 출금만 · practice 403 · session isolation · missing≠0 · known defect 0 · Money Rule 재정의 0 · web PendingFigma=8 유지) |
...
| execute-web-wire | Track B B-EXECUTION-001 — **live** (`useTradeExecution` · `/trades/[id]/execute` 실데이터 · cookie session tick · consumer 상태 테이블 · progressPct 표시 0 · 레거시 Canon 복구 0) |
| trades-web-wire | Track B B-TRADES-001 — **live** (`GET /api/v1/trades` session list · `@aipo/sdk/trades` fetchTradeList · `/trades` 최소 실데이터 · buckets.profitUsdt · progressPct 표시 0 · 레거시 Canon 복구 0) |
...
| krw-deposit-fx-semantics | Money §41.3 HC6-08 — **live** request-time quote · applied FX/credited USDT first-class · consumer own-read · crash-safe approve · Home current-FX unused · client calc 0 |
...
| home-product-contract | UI **Redesign R1 H4** `redesign-r1-home-product-contract`(ADR-018 Functional Authority · **historical/reference only 2026-08-18**) — standalone/manual(**not** T0/T1/T2-wired) · Visual Master 예시 리터럴 스캔은 과거 거버넌스 기록. 현재 VISUAL_TRUTH=APPROVED_FIGMA_ONLY. 실행 권위 0 |
