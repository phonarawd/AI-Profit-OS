...
| 경로 패턴 | verify |
|-----------|--------|
| `docs/product/consumer/**` · `docs/product/PUTDUK_PRODUCT_DESIGN_ENGINEERING_OPERATING_SYSTEM.md` · `tooling/verify/consumer-ux-architecture.cjs` · `tooling/verify/core-loop-contract.cjs` · `tooling/verify/participate-web-wire.cjs` · `governance/consumer-loop/**` · `apps/web/app/profits/[id]/**` · `apps/web/app/trades/**` · `packages/sdk/src/index.ts` · `packages/sdk/src/participate/**` | consumer-ux-architecture · core-loop-contract · participate-web-wire (B-PARTICIPATION-001 web/SDK preflight+participate 실배선 · execute/trades는 다음 슬라이스) |
...
| participate-web-wire | Track B B-PARTICIPATION-001 — **live** (`@aipo/sdk/participate` issuePreflight+postParticipate · `/profits/[id]` 실연결 · amountUsdt=requiredCapitalUsdt · 목록 POST 0 · execute/trades PendingFigma 유지) |
