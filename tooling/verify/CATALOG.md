...
| 경로 패턴 | verify |
|-----------|--------|
| `docs/product/consumer/**` · `docs/product/PUTDUK_PRODUCT_DESIGN_ENGINEERING_OPERATING_SYSTEM.md` · `tooling/verify/consumer-ux-architecture.cjs` · `tooling/verify/core-loop-contract.cjs` · `governance/consumer-loop/**` · `apps/web/app/profits/[id]/**` · `apps/web/app/trades/**` · `packages/sdk/src/index.ts` | consumer-ux-architecture · core-loop-contract (B-LOOP-001 Product/Visual/Implementation · web participate POST 0 재실측) |
...
| core-loop-contract | Track B B-LOOP-001 — **live** (Product/Visual/Implementation 계약 · `CONSUMER_CORE_LOOP_CONTRACT.md` + `governance/consumer-loop/core-loop.v1.json` · web/SDK participate POST 0 재실측 · Engine Rule 재정의 0 · release E2E=`B-LOOP-002`) |
