...
/**
 * Model the currently known recovery truth without provider mutation:
 * Engine NOT_ISSUED, isolated staging absent, DB hardening NOT_READY,
 * Render autoDeploy still enabled, and TRON HD vault unavailable.
 * A green Engine/artifact acceptance alone must never flip this to GO.
 */
{
  const x = basePass();
  x.engine = {
