/**
 * withdraw step-up runtime — WebAuthn 503 · 서버 이메일 · 원자적 consume · 만료 토큰.
 */
"use strict";

const hmacKey =
  process.env["JWT_USER_SECRET"] || "change_me_withdraw_stepup_runtime";
process.env["JWT_USER_SECRET"] = hmacKey;
...

  {
    ...
    const db = {
      async query(text, params) {
        if (text.includes("SET expires_at = to_timestamp(0)")) {
