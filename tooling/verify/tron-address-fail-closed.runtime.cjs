...
assert.throws(
  () => deriveTrc20Address({ secretRef: "ops:vault:hot-wallet-xpub", derivationIndex: 0 }),
  (err) =>
    err instanceof TronHdDerivationUnavailableError &&
    err.code === TRON_HD_DERIVATION_UNAVAILABLE,
);
...
assert.throws(() => deriveTrc20Address({ secretRef: "", derivationIndex: 0 }), /hotWalletXpubRef required/);
assert.throws(
  () => deriveTrc20Address({ secretRef: "ops:vault:hot-wallet-xpub", derivationIndex: -1 }),
  /derivationIndex/,
);

console.log(
  "[verify:tron-address-runtime] PASS (VALID_REFERENCE_FAILS_CLOSED · PERSIST_NOT_REACHED · NO_FAKE_SPENDABILITY)",
);
