/**
 * verify:rel-218-admin-growth-deposit
 */
const fs = require("fs");
...
const page = read("apps/admin/app/admin/growth/deposit/page.tsx");
const hub = read("apps/admin/app/admin/growth/page.tsx");
if (!page.includes('redirect("/admin/growth?tab=deposit")')) fails.push("deposit must redirect to hub tab");
if (!hub.includes('"' + "deposit" + '"') && !hub.includes("tab=deposit")) fails.push("hub missing deposit");
if (!hub.includes("adminGet")) fails.push("hub must live-wire");
if (!hub.includes("이 탭의 운영 목록 API가 없습니다.") && "deposit" !== "content") {
