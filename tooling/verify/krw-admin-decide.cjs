...
const files = [
  ...
  "services/api-nest/src/wallet/krw-deposit.money.ts",
  "services/api-nest/src/wallet/krw-deposit.fx.ts",
  "services/api-nest/src/wallet/krw-deposit.admin.controller.ts",
  "packages/sdk/src/wallet/fetch.ts",
  "supabase/migrations/20260818010000_krw_deposit_fx_facts.sql",
...
if (!(schema.required || []).includes("payableSuffixRole")) {
  fails.push("payableSuffixRole must be required");
}
if (!(schema.required || []).includes("quote") || !(schema.required || []).includes("final")) {
  fails.push("quote and final must be required (nullable objects)");
}
if (schema.properties?.payableSuffixRole?.const !== "bank_transfer_identification") {
  fails.push("payableSuffixRole const must be bank_transfer_identification");
}
...
const applySrc = read("services/api-nest/src/wallet/krw-deposit.apply.ts");
const rejectStart = applySrc.indexOf("async reject(");
const approveStart = applySrc.indexOf("async approve(");
if (rejectStart < 0 || approveStart < 0) {
  fails.push("approve/reject methods required");
} else {
  const rejectBody = applySrc.slice(rejectStart, applySrc.indexOf("\n  async ", rejectStart + 1));
  if (/postJournal/.test(rejectBody)) {
    fails.push("reject() must not postJournal");
  }
  const approveBody = applySrc.slice(
    approveStart,
...
if (!adminCtrl.includes("WALLET_ADMIN_ROUTES.krwDepositRequestById")) {
  fails.push("admin controller must bind krwDepositRequestById");
}
...
if (!userCtrl.includes("WALLET_USER_ROUTES.krwDepositRequestById")) {
  fails.push("user wallet must bind krwDepositRequestById");
}
if (!userCtrl.includes("listKrwDeposits") || !userCtrl.includes("getKrwDeposit")) {
  fails.push("user wallet must expose GET list/detail for own KRW deposits");
}
if (!userCtrl.includes("this.sessionUserId(req)")) {
  fails.push("user KRW deposit reads must use sessionUserId");
}
...
if (!routes.includes("krw-deposit-requests/:id")) {
  fails.push("consumer/admin KRW deposit detail route required");
}
