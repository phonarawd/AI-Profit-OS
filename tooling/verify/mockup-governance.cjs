...
const uiAuth = path.join(root, ".cursor/rules/ui-authority-governance.mdc");
if (!fs.existsSync(uiAuth)) {
  fails.push("missing .cursor/rules/ui-authority-governance.mdc");
} else {
  const authSrc = fs.readFileSync(uiAuth, "utf8");
  for (const needle of [
    "HARD INVARIANT",
    ...
    if (!authSrc.includes(needle)) {
      fails.push(`ui-authority-governance.mdc must declare: ${needle}`);
    }
  }
}
