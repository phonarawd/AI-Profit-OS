...
if (rebaseRequired) {
  if (!evidence.includes("CERT_ISSUED = 0") || !evidence.includes("STALE_PENDING_REBASE = 1")) {
    fails.push("REL-508 evidence must mirror REL-502 NOT_ISSUED / rebase pending");
  }
} else if (!evidence.includes("CERT_ISSUED = 1") || !evidence.includes("STALE_PENDING_REBASE = 0")) {
  fails.push("REL-508 evidence must mirror current-epoch REL-502 issuance");
}
if (rebaseRequired) {
