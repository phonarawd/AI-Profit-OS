...
const currentProduction = JSON.parse(
  fs.readFileSync(
    path.join(root, "governance/recovery/production-db-hardening-snapshot.20260903.v1.json"),
    "utf8",
  ),
);
const currentReadiness = compareSnapshot(currentProduction);
assert.equal(currentProduction.production_mutation, 0);
assert.equal(currentProduction.migration_ledger.applied_count, 43);
assert.equal(currentProduction.migration_ledger.latest_version, "20260902155632");
assert.equal(currentReadiness.ok, false);
assert.equal(currentReadiness.status, "NOT_READY");
assert.deepEqual(currentReadiness.app_default_acl_owners, ["postgres"]);
for (const blocker of [
  "rls_enabled_mismatch:push_control:expected=true:got=false",
  "rls_forced_mismatch:push_control:expected=true:got=false",
  "rls_enabled_mismatch:push_subscriptions:expected=true:got=false",
  "rls_forced_mismatch:push_subscriptions:expected=true:got=false",
  "forbidden_privilege:admin_audit_events:TRUNCATE",
  "forbidden_privilege:push_control:TRUNCATE",
  "forbidden_privilege:push_subscriptions:TRUNCATE",
]) {
  assert.ok(currentReadiness.fails.includes(blocker), "current Production blocker missing: " + blocker);
}
assert.ok(
  currentReadiness.warnings.includes(
    "managed_default_acl_outside_app_owner_scope:supabase_admin:anon",
  ),
);
