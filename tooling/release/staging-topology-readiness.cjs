...
function evaluateStagingTopology(snapshot) {
  ...
  if (!stageRender) {
    ...
    const candidateSha = String(s.candidate_sha || "").trim();
    if (!candidateSha) {
      blockers.push("candidate_sha_missing");
    } else if (nonEmpty(stageRender.source_sha) && stageRender.source_sha !== candidateSha) {
