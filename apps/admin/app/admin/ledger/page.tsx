    ...
    <main className="p-6 text-lux-text" data-testid="admin-ledger">
      ...
      {tab === "journal" && (
        <section className="mt-6 space-y-2">
          ...
          <p className="text-sm text-lux-text-muted">기록된 순서대로 보여 드립니다.</p>
          {!journals ? (
      ...
      {tab === "recon" && (
        <section className="mt-6 space-y-2">
          ...
          <p className="text-sm text-lux-text-muted">서로 연결된 금액 기록이 모두 맞는지 확인합니다.</p>
          {!recon ? (
            <p className="text-sm text-lux-text-muted">{T.admin.state.loading}</p>
          ) : !recon.ok ? (
        ...
        >
          ...
          <div className="mt-3 text-sm">
            {!shadow ? (
              ...
              <p className="text-lux-text-muted">아직 다시 확인한 기록이 없습니다.</p>
            ) : (
