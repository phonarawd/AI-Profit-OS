...
export default function Page() {
  ...
  return (
    <main className="p-6 text-pd-text" data-testid="admin-financial-page">
      ...
      <section className="mt-6" data-testid="financial-report-panel" data-report-api={reportApi}>
        {!report ? (
          ...
          <p className="mt-3 text-sm text-pd-text-muted">표시할 기간이 없습니다.</p>
        ) : buckets ? (
          <ul className="mt-3 space-y-3">
            {buckets.map((row, idx) => (
              <li key={readText(row.period) ?? String(idx)} className="rounded border border-pd-border p-3 text-sm">
