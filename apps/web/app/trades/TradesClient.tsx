...
export function TradesClient() {
  ...
  const profitState = profitError || profitUsdt == null ? "unavailable" : "ready";
  ...
  return (
    ...
    >
      ...
      <h1 className={styles.title}>{TITLE}</h1>
      <p className={styles.lead}>참여한 기회예요.</p>
      <dl className={styles.facts}>
        <div>
          <dt>수익</dt>
          <dd>{profitState === "ready" ? usdtLine(profitUsdt) : "—"}</dd>
        </div>
      </dl>
                ...
                >
                  ...
                  {settled ? (
                    <span className={styles.itemMeta}>
                      정산 수익 {usdtLine(item.settledProfitUsdt)}
                    </span>
                  ) : null}
                </Link>
              </li>
