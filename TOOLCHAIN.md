...
```powershell
pnpm verify:gate:fast      # commit 전 (T0 · ~30s · 같은 트리 재실행은 stamp skip)
pnpm verify:gate:push      # push 전 (T1 · infra+stubs · stub cache)
pnpm verify:gate           # CI / main (T2 · next+opennext+api-nest 포함 · stamp 0)
# 강제 재실행: $env:AIPO_GATE_NO_STAMP=1
...
- Husky: pre-commit → `verify:gate:fast` · pre-push → `verify:gate:push` (명령 유지 · 같은 트리 stamp hit만 skip)
