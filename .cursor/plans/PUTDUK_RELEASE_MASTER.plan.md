...
overview: SINGLE_EXECUTION_SSOT. PRE-LOCK 1 + REL 117 + POST 19 = 137 canonical tasks. pointer-only 0. PLAN_LOCKED=TRUE. REL-502 NOT_ISSUED (CURRENT_EPOCH_REISSUE_REQUIRED CERT_ISSUED=0 REBASE_REQUIRED=1 ACK_RECEIVED=0). REL-503 COMPLETED(historical). REL-504 COMPLETED(historical readiness). REL-505 COMPLETED. REL-508 COMPLETED. REL-506 COMPLETED(historical ALL_ALIGNED=0 CURRENT_RELEASE_ID_PINNED=0 PRODUCTION_ANNOTATED_TAG=0 WEB_VITALS_RUM=0). REL-507 COMPLETED. PLAN_QUEUE_FIRST_PENDING=REL-700. CURRENT_RELEASE_PRECONDITION=REL-502_CURRENT_EPOCH_REISSUE. HARD_STOP_AFTER=REL-603. REL-600 COMPLETED. REL-601 COMPLETED. REL-602 COMPLETED. REL-603 COMPLETED. REL-400..501 COMPLETED. BATCH_REL_200_206=ADMIN_ENTRY.
todos:
  - id: pre-lock-001
  ...
  - id: rel-125
    ...
    status: completed
  - id: rel-126
    content: "[Consumer][Legacy:C-ACC-001~003] Support(/me/support) 클로저 - deposit-disputes · fake chat 0"
    status: completed
  - id: rel-127
    content: "[Consumer][Legacy:C-ACC-001~003] Guides(/me/guide/* 7라우트) 클로저 - AccountFrame · leftover chrome 0"
    status: completed
  - id: rel-128
  ...
  - id: rel-215
    ...
    status: completed
  - id: rel-400
    content: "[Hardening] Admin Control Plane -- DONE spec only, 3-mode locked, implementation 0"
    status: completed
  - id: rel-213
    content: "[Admin] /admin/system-control -- DONE live kill reads + preview/confirm, unpublished honest empty"
    status: completed
  - id: rel-214
    content: "[Admin] /admin/audit -- DONE honest empty + reserved rbac tab, delete UI 0"
    status: completed
  - id: rel-401
...
PLAN_LOCKED = TRUE
BLOCKING_ON = [REL-502_CURRENT_EPOCH_REISSUE]
PLAN_QUEUE_FIRST_PENDING = REL-700
FIRST_EXECUTION_TODO = REL-700
CURRENT_RELEASE_PRECONDITION = REL-502_CURRENT_EPOCH_REISSUE
REL_502_CURRENT_TRUTH = NOT_ISSUED
CERT_ISSUED = 0
REBASE_REQUIRED = 1
ACK_RECEIVED = 0
PROTECTED_SCOPE_DRIFT = 1
LAST_COMPLETED_TODO = REL-603
BATCH_REL_200_206 = ADMIN_ENTRY
HARD_STOP_AFTER = REL-603
PRE_LOCK_COUNT = 1
REL_COUNT = 117
POST_COUNT = 19
MASTER_TODO_COUNT = 137
REVISION = Overnight v4 SSOT truth repair (count=137 REL-502 NOT_ISSUED current epoch)
...
이 파일 하나만 읽어도 137개 실행 TODO를 수행할 수 있다.
PLAN_QUEUE_FIRST_PENDING=REL-700 이더라도 CURRENT_RELEASE_PRECONDITION=REL-502_CURRENT_EPOCH_REISSUE 를 건너뛰면 무효.
REL-503~508 COMPLETED 는 historical/certification task 완료이며 current release candidate READY를 의미하지 않는다.
특히 REL-506: ALL_ALIGNED=0 · CURRENT_RELEASE_ID_PINNED=0 · PRODUCTION_ANNOTATED_TAG=0 · WEB_VITALS_RUM=0 (은폐 금지).
...
STATUS: NOT_ISSUED
CURRENT_EPOCH: REBASE_REQUIRED
CERT_ISSUED: 0
ACK_RECEIVED: 0
PROTECTED_SCOPE_DRIFT: 1
NOTE: historical certification may exist; current-authoritative ISSUED=0 until ENGINE_ACCEPTANCE_REBASE_V1 on sealed release SHA
SOURCE_PLAN: ai_profit_os_02_5_engine_acceptance_c3d4e5f6.plan.md
SOURCE_TODO_IDS:
  - rel-502
  - qa0
...
EVIDENCE: governance/release-master/REL-601-STAGING-REGRESSION.md
...
```yaml
ID: REL-603
TITLE: Automated age-band usability cohort (Founder B)
STATUS: COMPLETED
SOURCE_PLAN: 퍼뜩_03_ui_ux_master_plan_be3fa3cc.plan.md (intake)
SOURCE_TODO_IDS:
  - rel-603
...
CURRENT_SCOPE: 9 automated cohort runs on staging preview. AUTOMATION_LEVEL=A2.
DEPENDENCIES:
  - REL-601
IMPLEMENTATION_STEPS:
...
ACCEPTANCE: 9 cohort x 4 scenario PASS; FAKE_PASS 0; human 0
EVIDENCE: governance/release-master/AGE_SPOTCHECK.md
EXIT_GATE: committed spec + verifier required
AUTOMATION_LEVEL: A2
PROTECTED_SCOPE_MUTATION: false
```
...
현재 실행 포인터: PLAN_QUEUE_FIRST_PENDING=REL-700 · CURRENT_RELEASE_PRECONDITION=REL-502_CURRENT_EPOCH_REISSUE · REL_502_CURRENT_TRUTH=NOT_ISSUED. HARD_STOP_AFTER = REL-603. LAST_COMPLETED_TODO = REL-603.
