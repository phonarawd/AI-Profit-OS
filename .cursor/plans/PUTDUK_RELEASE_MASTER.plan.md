...
EVIDENCE: apps/admin/app/admin/opportunities + tooling/verify/rel-210-admin-opportunities.cjs + governance/release-master/REL-210-OPPORTUNITIES.md
EXIT_GATE: 유저 JWT로 200이면 FAIL
AUTOMATION_LEVEL: A3
PROTECTED_SCOPE_MUTATION: false
```
...
EVIDENCE: apps/admin/app/admin/adapters + tooling/verify/rel-211-admin-adapters.cjs + governance/release-master/REL-211-ADAPTERS.md
EXIT_GATE: 유저 JWT로 200이면 FAIL
AUTOMATION_LEVEL: A3
PROTECTED_SCOPE_MUTATION: false
```
...
EVIDENCE: apps/admin/app/admin/support + tooling/verify/rel-212-admin-support.cjs + governance/release-master/REL-212-SUPPORT.md
EXIT_GATE: 유저 JWT로 200이면 FAIL
AUTOMATION_LEVEL: A3
PROTECTED_SCOPE_MUTATION: false
```
...
EVIDENCE: "apps/admin/app/admin/ai-logs + governance/release-master/REL-215-AI-LOGS.md + verify:rel-215-admin-ai-logs"
EXIT_GATE: 유저 JWT로 200이면 FAIL
AUTOMATION_LEVEL: A3
PROTECTED_SCOPE_MUTATION: false
```
...
현재 실행 포인터는 헤더의 `FIRST_EXECUTION_TODO = REL-400`. HARD_STOP_AFTER 는 소비됨(`[]`).
REL-300~REL-305는 2026-08-22 AI Coach Quality/Safety Batch에서 기존 P/G/S 오너+committed fixture 실행 후 COMPLETED.
REL-215는 2026-08-22에 기존 `ai_logs` 오너+AdminGuard를 `/admin/ai-logs`에 실배선 후 COMPLETED. Runtime/browser QA는 이 슬라이스에서 강제하지 않음(BLOCKED_LOCAL_RAM). REL-400은 착수하지 않음.
