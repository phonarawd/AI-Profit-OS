...
overview: v7.24.9 PART0~9 completed 불변 · UI_UX_ENTRY_GATE=OPEN · ADR-018 Peotteok Visual Master Reset · R1 Home H1~H7 completed · Part B V2 completed · first pending=trust-age-spotcheck · 상세 patch notes=본문 v7.24.x 절
todos:
  ...
  - id: redesign-r2-acquisition-contract
    content: "[grok-4.5|256K] Redesign R2-1 · /l/[variant]·/ads*·/auth/login|signup|complete-profile·/onboarding Product/Visual/Implementation Contract+wire+copy/API mapping · utility→capital disclosure·Nest JWT/Kakao/Passkey·consent·recovery · existing PART2/Infra contracts keep|adapt · 새 route0 · **v7.24.5 HARDEN(§6.4c.1 I · landing-3s-ui 재오픈0):** 카피 hierarchy=Hero비교기능·Benefit확인편의·Opportunity signal=`가격 차이 확인`(수익약속0)·CTA=기존잠금(`실시간 시세 맵 열기`/`지금 비교해 보기`/`시세 맵 보기`) · 「시세 맵」=비교화면 라벨(지리맵 발명0) · Infra §31.2d G9「가입 전 시세 맵」출시 금지 · legal SSOT 내용수정0·가짜 legal 0 · 기존 `/me/legal` read-only public alias 최소검토만 · Core(`/profits/*` `/wallet/*` `/me/*` 비공개)는 인증 가드(robots≠보안) · dual-layer=Public Ad vs Authenticated Core(reviewer/UA 분기0) · `/ads` `/l/*` Admin 변경0"
    status: pending
  - id: redesign-r2-acquisition-implementation
    content: "[grok-4.5|256K] Redesign R2-2 · R2 contracts 기준 Landing/Auth/Profile/Onboarding gap-only 구현 · existing GuestChrome/utility copy/consent/session-cookie/PART9 보존 · duplicate landing/auth component0 · 320~2560+/a11y/perf · **v7.24.5 HARDEN:** 갭-only · 맵 컴포넌트 발명0 · `/ads` `/l/*` 동일 surface 유지·Admin route 변경0 · 새 라우트0(legal public alias는 기존 legal SSOT 읽기전용일 때만) · 광고 카피 자유생성 0(허용 축 PRICE_COMPARISON/MARKET_DISCOVERY/TIME_SAVING 안에서만·AI/알림은 공개 플로우에 실UI 있을 때만)"
    status: pending
  - id: redesign-r2-acquisition-certification
    content: "[grok-4.5|256K] Redesign R2-3 · onboarding-experiential/auth-surfaces/landing-3s/marketing-compliance/operator-footer/auth-session-cookie + verify:acquisition-release 신설+CATALOG · actual guest/auth/error/resume · known P0~P3 defect0 · **v7.24.5 HARDEN:** verify:marketing-compliance=기존 유지+L1 INTERNAL_HIGH_RISK_AD_COPY_LEXICON(공식 금지어 주장0·legal/core 스캔0)+L2 claim-pattern+L3 semantic fixture+ad↔landing consistency · Human Review=Infra §31.2d 유지 · verify:acquisition-release에 광고랜딩→Core 비의도 redirect 0"
    status: pending
...
> **v7.24.6 (Cursor Ops / Ads Orchestrator / Founder Dashboard · 플랜+rule only · 구현0):** 03 File-Serial/R2 HARDEN **불변** · todo ADD 0 · Founder Dashboard Owns=04 `admin-ops` · Orchestrator/Standing Authorization/Cursor Ops Owns=06 가산 · `landing-3s-ui` 재오픈 0 · dual-layer/legal SSOT 변경 0  
> **v7.24.5 (Ads Additive Patch · 플랜+rule only · 구현0):** R2 Acquisition 3 todo 인수조건 HARDEN — 카피 hierarchy·시세맵 라벨·G9 출시금지·legal public alias 검토(SSOT 내용 수정0)·Core 인증 가드·`verify:marketing-compliance` L1+L2+L3+consistency · `landing-3s-ui` completed **재오픈 0** · `/ads` `/l/*` Admin 변경 0 · programmatic SEO 0  
...
> | H6 | New Implementation Contract | `redesign-r1-home-implementation-contract` **completed**(2026-08-16 · v7.24.8 · `peotteok-home-implementation-contract.v2.md`) · KEEP/REWIRE/REMOVE_FROM_RUNTIME/REPLACE/INVESTIGATE forensic 완료 |
> | H6.5 | contract sync | `redesign-r1-home-contract-sync` **completed**(2026-08-16 · v7.24.9 · `peotteok-home-contract-sync.v1.md` · blocking contradiction=0 · H7_CONTRACT_READY=YES · H7_RUNTIME_START_ALLOWED=NO) |
> | — | Visual Master V2 Rebase + H5/H6/H6.5 Delta Sync | `redesign-r1-home-visual-master-v2-rebase` **completed**(2026-08-16 · v7.24.10 · `peotteok-home-visual-master-intake.v2.md`+`peotteok-home-v2-delta-sync.v1.md` · Founder Home visual direction 업그레이드 · V1 intake SUPERSEDED banner만 추가(본문 삭제 0) · H5/H6/H6.5 재분류 필요=0건 · HOME_VISUAL_MASTER_V2_REBASE=PASS · H7_CONTRACT_READY_AFTER_V2=YES · ASSET_PART_B_V2_READY=YES) |
> | — | Home visual asset production(brand-assets Part B) | `redesign-r1-home-visual-asset-production` **completed**(2026-08-16 · v7.24.11 · Part B V2 · `peotteok-home-asset-production-part-b.v2.md` · 8행 종결 · ASSET_PART_B_V2_COMPLETE=YES · H7_RUNTIME_START_ALLOWED=YES · H7 착수 0 · **SSOT align:** SVG root 64×64 SHA search=`6db9648d…2740` chart=`64772e60…7935` clock=`e666b863…98b1` · wire V2 authority · H7_STARTED=NO) |
> | H7 | presentation implementation | `redesign-r1-home-implementation` **completed**(2026-08-16 · H7_PRESENTATION_IMPLEMENTATION=PASS · `h7ImplementationStatus=COMPLETE`) · 선행=H6.5 · **UI_UX_ENTRY_GATE=OPEN** · **PASS=구조/도메인 게이트이지 시각 승인·LOCK이 아님** · todo 재오픈 없이 프레젠테이션 교정 가능(`ui-authority-governance.mdc`) |
> | H8 | verification | 별도 todo 없음 — H7 완료조건 내 domain verify(T0/T1/T2)로 수행 · **Canon/domain PASS ≠ 시각 승인** |
...
New Implementation Contract 작성 전, 기존 화면의 component/hook/handler를 전수 분류한다 — **분류 없이 추측으로 새 컴포넌트를 추가하지 않는다.**
KEEP은 기능/데이터/보안 보존이다. 프레젠테이션이 Visual Master와 다르면 `REPLACE`하거나 페이지 프레젠테이션을 수정·교체할 수 있다.
H6 이후에도 Visual Master 미충족이면 분류를 갱신하고 프레젠테이션을 재구축할 수 있다(`.cursor/rules/ui-authority-governance.mdc`).
...
- R2 Acquisition: Landing/Auth/Profile/Onboarding. utility→capital 경계와 consent/Auth recovery를 보존한다. **v7.24.5 HARDEN pointer=§6.4c.1 I** (카피 hierarchy·시세맵 라벨·legal alias 검토·Core 인증 가드·compliance 3층). `landing-3s-ui` 재오픈 0.
...
#### I) R2 Acquisition HARDEN (v7.24.5 · **landing-3s-ui 재오픈 0** · A~H 완료 이력 불변)

> **Owns:** `redesign-r2-acquisition-contract` / `implementation` / `certification` · Infra §31.2d·§31.7 pointer  
> **금지:** PART2c completed 재실행 · `/ads` `/l/*`를 Admin으로 변경 · legal SSOT 문장 삭제/왜곡 · 가짜 심사용 legal · 지리 시세맵 발명 · programmatic SEO

| 항목 | 인수조건 |
|------|----------|
| 카피 hierarchy | Hero=실제 비교 기능 · Supporting=시세/가격 변화 확인 · Benefit=여러 사이트를 돌아다니지 않고 확인 · Opportunity signal=`가격 차이 확인`(수익/보장/시간+금액 약속 **0**) · CTA=기존 잠금 라벨 |
| 「시세 맵」 | **시세·가격 비교 화면**의 제품 라벨 · 지리 지도 UI 발명 **0** · 소재는 실비교 UI(PriceCompareMargin/비교 카드) |
| 허용 메시지 축 | `PRICE_COMPARISON` · `MARKET_DISCOVERY` · `TIME_SAVING` · AI/알림/기회참여는 공개 광고 플로우에 실UI가 있을 때만 |
| G9 | Infra §31.2d `가입 전에 시세 맵부터 볼 수 있어요` = **출시 금지**(CTA 목적지=`/onboarding` · 가입 전 공개 맵 라우트 0) |
| legal | 기존 `T.legal.*` / `/me/legal` **내용 수정 0** · 공개 접근 gap이 있으면 **read-only public alias** 최소 검토만 |
| Core 가드 | `/profits/*` · `/wallet/*` · `/me/*`(legal public alias 예외) = **인증/인가** · robots.txt를 보안으로 사용 **0** |
| dual-layer | Public Ad Product Surface vs Authenticated Product Core · **Reviewer vs Real User 분기 0** · UA/IP로 다른 히어로 **0** |
| CI | 기존 `verify:landing-3s`+`verify:operator-footer` 유지 · `verify:marketing-compliance` = L1+L2+L3+ad↔landing consistency(Infra §31.7 HARDEN) · Human Review=§31.2d |
...
| MUP2 | 목업 색/여백 복제로 깨짐 | 전역 토큰 우선 · 미승인 hex hardcode Fail · **승인 Master 실측을 전역 토큰이 못 내면 페이지 전용 semantic token 허용** |
...
> **예외(2026-08-16 ADR-018):** 위 MUP1~MUP8 및 아래 33.8.5의 절대 규칙은 **Founder-approved 이며 ADR-018 §9로 등록된 Visual Master surface**에는 그대로 적용되지 않는다 — 그 화면은 LOCK 전이라도 pixel-accurate 구현이 **의무**다(`.cursor/rules/visual-master-intake.mdc` · `ui-authority-governance.mdc`). `visual-locks` LOCK은 Founder visual approval **이후** 등록이며, LOCK 부재가 수정 금지를 의미하지 않는다. 일반 미승인 mockup 규칙(픽셀 diff 금지·"목업이랑 똑같이" 거부·사진 불일치=비결함)은 **그 외 모든 화면**에서 계속 100% 유효하다.

#### 33.8.5 에이전트 운영 규칙 (중복0)

1. UI 작업 시 **사진 목업을 열지 않음** (기본). 열어도 구조 의도만.  
2. 구현 전 Canon wire + Brand + Lux 확인.  
3. 미승인 사진에 “목업이랑 똑같이” 요청 → **Canon/토큰 기준으로 재해석** 후 구현 (미승인 픽셀 맞추기 거부).  
4. 미승인 사진-픽셀 불일치 = **비결함** (ADR-013). 승인 Visual Master 화면의 Visual Contract 불일치 = **결함**(LOCK 전에도).  
5. **예외(ADR-018):** 대상 화면이 Founder-approved Visual Master로 등록되어 있으면 위 1~4가 아니라 `visual-master-intake.mdc`의 pixel-accurate 구현 규칙을 따른다 — LOCK 전에도 Visual Contract 불일치는 **결함**이다.

#### 33.8.6 UI Authority Governance (2026-08-16 · 권위 강도 분리)

SSOT = `.cursor/rules/ui-authority-governance.mdc`.
completed todo YAML의 역사적 문구(「분류 없이 새 컴포넌트 추가 금지」·「기존 functional runtime 보존」등)는
H6/H7 **작성 세션** 기록이다. 현재 권위는 본 절과 `ui-authority-governance.mdc`가 우선한다.

- 승인 Visual Master > 기존 컴포넌트/현재 구현. 현재 구현은 SSOT가 아니다.
- HARD INVARIANT(Money/Engine/Auth/API)는 유지. 프레젠테이션은 Visual Master 충족을 위해 수정·재구축 가능.
- H7 코드 수정 금지의 범위 = 도메인 계약 + 잠긴 증거. Home 프레젠테이션은 포함하지 않음.
- Canon PASS ≠ 시각 완료. LOCK 전에도 승인 Master 화면의 Visual Contract 불일치 = 결함.
- 인증 Home과 게스트 Home은 다른 상태. `MISSING_VISUAL_ASSET` ≠ 완료(`VISUAL_REVIEW_REQUIRED`).
