...
| 축 | v7.23 실행값 | 폐기·보존 |
|---|---|---|
| Consumer theme | **ADR-017 `peotteok-light` Light+Purple 단일 출시 테마** | Dark theme=archive/legacy · dual toggle 0 |
...
> **툴체인 SSOT:** **ADR-015** — Node22 · **pnpm only** · **next@16** · **Tailwind v4**+visual-system `@theme` · Rust · Compose=옵션 · npm/bun install 금지  
...
| **visual-system-Fintech** | Deep Obsidian · Tier Motion · G4 ticker/counter |
...
| **v7.22.4 목업 거버넌스 흡수** | **ADR-013** — 사진목업=intent archive only · 시각복제 금지 · Canon=visual-system+Brand+컴포넌트+구조와이어 · 충돌시 코드/토큰>플랜>Canon>사진 · `verify:mockup-governance` · Cursor rule alwaysApply |
...
| **v7.22.6 그린필드 툴체인 흡수** | **ADR-015** — **next@16**(15 pin 폐기) · **Tailwind v4**+visual-system `@theme` · **pnpm@10.14** only · Node22 · rust-toolchain · Compose PG17/Redis · `TOOLCHAIN.md` · `verify:stack-lock` 작업전 PASS · npm/bun install 금지 |
...
| **v7.22.11 온보딩·인증·광고·KYC 흡수** | UI §6.4 체험형+Canon · Infra §31.2a `/ads` alias · §31.2b 3초 예산 · §51.9.1 Stage A/B 필드 · Money §42 visual-system 3면+`kyc-submission.v1` · **주민번호 타이핑 0** · 소유권 SSOT 1곳+pointer |
...
| **v7.22.17 PWA 결함 0화** | PWA `05` next@16 · Push Phase0 in-process · visual-system theme · WebAuthn Owns=Money §43 · §24 Infra pointer · Admin push kill · CATALOG `verify:pwa-*` · 이중본 sync · Canon install/offline |
...
| SEO / Organic | 8.5 |
| visual-system-Fintech Motion | 9.0 (tier + reduced-motion) |
...
- `27` — marketing·SEO
- `28` — visual-system-Fintech visual·motion (**G4 ticker/counter · §35**)
...
1. **M0** — CONSTITUTION 22~28·35~51 (+§47.12~14 · §50.9) + schemas + **supabase/migrations**(+vector) + brand + visual-system `@theme` + monorepo-skeleton  
...
| MS | 내용 |
|----|------|
| M0 | Constitution + schemas/migrations + visual-system + monorepo |
...
- **CONSTITUTION 28 + visual-system components + tier motion**
...
| `packages/ui/components/visual-system/` | CountUp, Ticker, MotionCTA, Receipt |
...
| **ADR-013 Mockup Governance** | 사진/PNG 목업 = **intent archive only** · 구현 시각 SSOT = visual-system tokens + Brand Kit + `packages/ui` + Canon wire · 충돌 시 **코드/토큰 > 플랜 > Canon > 사진목업** · UI §33.8 | 사진 픽셀 복제 · 목업별 로고/색/탭 drift · “목업이랑 똑같이” 픽셀 QA |
...
| **ADR-015 Greenfield Toolchain** | **Node22** · **pnpm@10.14.0 only** · **next@16** · **Tailwind v4**+visual-system `@theme` · Rust · OpenNext/CF · `TOOLCHAIN.md` · Compose=옵션 | next@15·TW3 · npm/bun SSOT |
