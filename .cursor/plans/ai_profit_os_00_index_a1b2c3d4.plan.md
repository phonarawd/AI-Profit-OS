...
7. **06 R7~R8**: Marketing/Auth/adapter binding → Backend/Data certification → Infra/Observability/Release certification → **v7.24.6 APPEND** Ads Provider / Standing Authorization / Orchestrator / Cursor Autonomous Ops → **v7.24.7** `ads-autonomous-ops-release-certification` (기존 1~6 상대 순서 불변 · R8 ≠ 최종 자동운영 출시 판정).
...
| 06 | Infra & Marketing | Marketing/Auth/adapter + **v7.23 R7/R8** + **v7.24.6 ads/cursor 가산(R8 이후)** + **v7.24.7 최종 자동운영 certification** | `ai_profit_os_06_infra_a7b8c9d0.plan.md` |
...
| **마케팅·SEO** | 매체별 랜딩·Server CAPI·UTM→입금·IndexNow · **v7.24.6** Ads Orchestrator≠CAPI · Standing Authorization · Founder Dashboard=`/admin` |
...
| **v7.24.5 Ads Additive Patch (플랜+rule · 구현0)** | 03 R2 / 04 admin-ops / 06 Marketing7+R8 **HARDEN** · CAPI outbox·stable event_id·delivery state · marketing-compliance L1+L2+L3+consistency · `/ads` `/l/*` crawlable+noindex · cloaking 0 · completed 재오픈 **0** · 07 **0** |
| **v7.24.6 Cursor Ops / Ads Orchestrator / Standing Authorization (플랜+rule · 구현0)** | TRUE GAP **ADD** 06 R8이후=`ads-provider-onboarding`→`ads-budget-standing-authorization`→`ads-campaign-orchestrator`→`cursor-autonomous-ops-enablement` · Founder Dashboard **ABSORB** 04 `admin-ops` §9.5.8 · OpsEvent **ABSORB** `infra-observability-late` · Audit **ABSORB** · CAPI≠Orchestrator · Budget Guardrail=deterministic · production `workflow_dispatch` HUMAN 유지 · File-Serial/dual-layer/legal/completed **불변** · Skill/Agent/Automation/Cloud/Bugbot/MCP enable **0** · 07 **0** |
| **v7.24.7 최종 자동운영 출시 판정 분리 (플랜+rule · 구현0)** | 06 마지막 ADD `ads-autonomous-ops-release-certification` · R8 Core Infra 의미/완료조건 **불변** · R8 PASS ≠ 「퍼뜩의 최종 자동운영 출시 준비 완료」 · 4개 가산 enablement completed ≠ 그 판정 · 07 **0** · completed 재오픈 **0** |
