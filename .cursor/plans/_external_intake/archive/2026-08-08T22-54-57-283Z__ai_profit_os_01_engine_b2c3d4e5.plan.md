...
overview: "v7.22.24 Engine·멤버십등급·AI해금·매칭엄격도·유저별override. Index=00."
todos:
  ...
  - id: match-strictness-policy
    content: "[grok-4.5|256K] §48.13.3 matchStrictness→policy 맵·난수0·멤버십×밴드 오버레이·verify:match-strictness"
    status: pending
  - id: market-intel-engine
    content: "[grok-4.5|256K] §0.0 Signup-Ready 6 only + Asset Master + Rust 경계 + §36/§43 · 금지 adapter 코드경로 0"
    status: pending
  - id: signup-ready-adapters
    content: "[composer-2.5|200K] ebay+yahoo-jp+pokemontcg+ygoprodeck+coingecko+frankfurter 워커·쿼터·캐시·출처표시 (어댑터별 슬라이스)"
    status: pending
  ...
  - id: luxury-bag-vertical
    content: "[composer-2.5|200K] luxury_bag 시드·Asset Master 이미지·ebay+yahoo_jp 호가·필터칩 가방"
    status: pending
  ...
  - id: match-success-rule-engine
    content: "[grok-4.5|256K] §48.13+§48.13.1~3 participate↔Rule·strictness·golden · verify:match-success-rule/match-strictness"
    status: pending
...
> **단일 편집본:** 이 해시 파일만 · `C:\Users\PC\.cursor\plans` 와 워크스페이스 `.cursor/plans` **동일 내용 유지**
...
|------------|------|------|------|-----------------|----------------|
| `ebay` | developer.ebay.com 무료 | **실호가 Listing** (buy/sell leg) | watch + trading_card + luxury_bag | Browse API `item_summary/search` (+ image) | ~5k calls/day · Redis cache · 유저요청 시 외부호출 금지 |
| `yahoo_jp` | developer.yahoo.co.jp AppID | **실호가 Listing** (JP) | watch + trading_card + luxury_bag | Auction Web API search (+ 허용 썸네일) | 일일 한도 준수 · backoff |
...
Asset Master (수동 시드 · imageUrl 포함)
  ...
  → ebay + yahoo_jp 로 Listing/PriceObservation (+ listing 썸네일 후보)
  ...
  → Opportunity (출처 수·staleAt·adapter_id·assetImageUrl 표시)
  → Redis → DO/SSE → 유저 UI
```

**가드:**
- 유저 클릭 경로에서 외부 API 호출 **금지** (캐시 miss 시 stale 표시 또는 백그라운드 refresh 큐)  
- Opportunity 자동 공개: **listing 소스 ≥1** (권장 2: ebay+yahoo) + fresh + 이상치 필터 + **§0.0.6 이미지 가드**  
- `pokemontcg`/`ygoprodeck` 가격만으로 자동 공개 **금지** (참고가 ≠ 체결 가능 호가)  
- Admin override는 listing 부재 시에도 가능 (출처=`admin`) · 이미지 없으면 Admin R2 업로드 필수
...
interface AssetMasterImage {
  ...
  imageSource: 'ebay' | 'yahoo_jp' | 'pokemontcg' | 'ygoprodeck' | 'admin_r2';
...
3) 주 listing 썸네일 (ebay Browse image / yahoo 허용 썸네일)
...
├── ebay-adapter            # §0.0 ACTIVE · Browse API · watch|trading_card|luxury_bag (+image)
...
interface OpportunityCard {
  ...
  // 상품 (보조, 작게) — 썸네일은 §0.0.6
  category: 'watch' | 'trading_card' | 'luxury_bag';  // v1 수직 · 마켓 탭 필터
  ...
  assetImageSource: 'ebay' | 'yahoo_jp' | 'pokemontcg' | 'ygoprodeck' | 'admin_r2';
...
| Type | v1 | 예 |
|------|-----|-----|
| price | ✅ | Patek/AP/Rolex 등 하이엔드 시계, PSA·TCG 카드, 명품 가방 · iPhone/LEGO = **v2+** |
| fx | ✅ | USD/JPY/EUR |
| benefit | v2 | 카드·상품권·쿠폰 |
| limited | ❌ v1 | Nike 등 — **v2+** (adapter 준비 전 코드경로 0) |
| resale | ❌ KR 제외 | 한국 C2C 비교 **영구 제외** · 해외 리세일만 별도 type로 재정의 시 ADR 필요 |

**v1 홈/수익 피드:** `status=available` + adapter live + `executionMode==='orchestrate'` + `compareReady` 만 노출.
