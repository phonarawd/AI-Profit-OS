...
overview: 기존 `.cursor/plans/PUTDUK_CURRENT_MASTER.plan.md`(Track A-G)를 CONTENT_AUTHORITY로 유지한 채, 그 상위에 새로운 단일 실행 권위 `.cursor/plans/PUTDUK_RELEASE_MASTER.plan.md`를 신설한다. `.cursor/plans/*.plan.md` 21개 전량을 실제로 읽고 todo 단위로 재판정(VERIFIED_DONE/MIGRATED_TO_REL/MIGRATED_TO_POST-CORE/SUPERSEDED/ARCHIVE)했으며, 이 과정에서 발견한 실제 갭(Admin RBAC/Kill-Switch/Price Override/R6인증, Production Migration Apply, Security/RLS baseline, 연령대 수동 실사)을 REL-405~409/504/603으로 신규 추가했다. Round5에서 사용자가 회수한 Figma fileKey(w7Yg8j2x9evuheOSSLqFw5)를 실제 MCP로 실사(15 frame 전수 분류, APPROVED_AUTHORITY 0건)했고, REL-504를 readiness-check로 재정의하며 실제 production DB apply를 REL-701-DB로 분리해 총 109개 REL-XXX로 확정했다. PLAN_LOCKED는 사용자 최종 승인 전까지 FALSE로 유지한다(PRE-LOCK-001은 이번 라운드에 실행 완료).
todos:
  - id: pre-lock-001
  ...
  - id: rel-122
    ...
    status: pending
  - id: rel-123
    content: "[Consumer][Legacy:C-ACC-001~003] Profile(/me) 클로저 - Track C \"핵심 8영역\" completed 선언 범위(재확인 게이트만)"
    status: pending
  - id: rel-124
  ...
  - id: rel-212
    ...
    status: pending
  - id: rel-213
    content: "[Admin] /admin/system-control(kill-switch) 실구현(REL-400 선행)"
    status: pending
  - id: rel-214
    content: "[Admin] /admin/audit 실구현(REL-400 선행)"
    status: pending
  - id: rel-215
    content: "[Admin] /admin/ai-logs 실사+구현(REL-300 선행)"
    status: pending
  - id: rel-216
...
```text
PLAN_LOCKED = FALSE
BLOCKING_ON = [PENDING_USER_FINAL_REVIEW]  (Figma blocker 해제 · Legacy Atomic Accounting 완료)
TOTAL_REL = 115 (Round6: Admin gap 3건[REL-222~224] + Production cert gap 3건[REL-505~507] 신규)
TOTAL_POST = 19 (Round6 신규: POST-001~019, 12+필드 완비 - Marketing/CAPI/SEO 7 + Ads자동화 5 +
  Admin post-core 4 + PWA v2 2 + Phase1 adapter 1)
REVISION = Round 6 (21개 legacy plan 309개 atomic todo 전수 파싱+5분류(VD/DUP/INV/SUP/CWR)+
  traceability table + mechanical reconciliation + pointer-only 0 완료)
ALL_LEGACY_PLANS_SCANNED = TRUE (21/21 파일 실제 Read, frontmatter 요약 아님)
ALL_LEGACY_ATOMIC_TODOS_EXTRACTED = TRUE (309/309, §I.4 GRAND TOTAL 검산 PASS)
FIGMA_URL_BLOCKER = 0 (fileKey 확보 + MCP 실접근 검증 완료)
```
...
나머지(REL-000~002,004~007,009,011~014,017~019,023, REL-100~130, REL-200~221, REL-300~305, REL-400,402~404,409, REL-505~507)는 **현재 시점 기준 FALSE**(apps/web·apps/admin·tooling·문서·governance만 건드림, 또는 순수 검증/집계라 protected root를 수정하지 않음). 단, 실행 중 예상 밖의 backend 변경이 필요해지면 그 즉시 해당 REL에 TRUE를 재부여하고 REL-502 집합에 편입한다(동적 재평가 - 사전 고정 아님).
...
Consumer(REL-100~130, 34 surface + 6 grouped-route) / Admin(REL-200~221, 22 route/21 surface) 매트릭스 기본 구조는 라운드2와 동일. 이번 라운드는 PRE-LOCK-001 실사 결과로 FIG 컬럼과 Home 반응형 필드를 실측값으로 갱신한다(더 이상 TBD 아님, 해당 화면만):
...
--- Round6 신규: TOTAL_REL/POST 정합성(전수 재계산) ---
HEADER_TOTAL_REL = 115
VALIDATOR_TOTAL_REL = 115
ACTUAL_REL_COUNT(frontmatter `- id: rel-` grep 실카운트) = 115
HEADER_TOTAL_POST = 19
VALIDATOR_TOTAL_POST = 19
ACTUAL_POST_COUNT(frontmatter `- id: post-` grep 실카운트) = 19
STALE_TOTAL_REL_COUNT = 0 (109 -> 115, REL-222~224[Admin 3건]+REL-505~507[Production cert 3건]
  신규 추가를 헤더/Validator/실카운트 3자 동시 반영)

REL 구간별 개수(Round6 갱신):
  Foundation    REL-000~023               = 24
  Consumer      REL-100~130               = 31
  Admin         REL-200~224(신규 222~224) = 25
  AI Capability REL-300~305               = 6
  Hardening     REL-400~409               = 10
  QA-Final      REL-500~507(신규 505~507) = 8
  Staging       REL-600~603               = 4
  Production    REL-700,701-pre,701-db,701,702,703,704 = 7
  합계 = 24+31+25+6+10+8+4+7 = 115 (OK, 헤더/Validator/실카운트 3자 일치)
...
--- AUTOMATION_LEVEL 재집계(REL 115 기준, 구간별 산출) ---
구간별 A-level 분해:
...
A3 = 23+31+25+6+10+5+1+5 = 106
A2 = 3(QA-Final)+2(Staging) = 5
A1 = 1(Foundation)+2(Production) = 3
A0 = 1(Staging)
검산: 106+5+3+1 = 115 (TOTAL_REL과 일치)
...
VALIDATOR_VERDICT = 구조적 PASS. 잔여 unlock 조건은 §G(변경 없음: 사용자 최종 승인만 남음).
```

## G. PLAN_LOCKED = FALSE 유지 조건(Round5: 조건1 충족 완료, 조건2만 잔존)

```text
UNLOCK_REQUIRES (AND):
  1. PRE-LOCK-001 완료 - [COMPLETE] fileKey 확보 + MCP 실사 + 15 frame 분류 + Registry 저장 완료
  2. 이번 라운드5 개정본(TOTAL_REL 정합성/Engine Acceptance 속성화/REL-504 분리/
     Home 반응형 정밀화/Figma 실사)에 대한 사용자 최종 검토 승인 - [PENDING]
```

## H. 실행 순서(Round5: PRE-LOCK-001 완료 반영)

1. (완료) Figma fileKey 실사 - PRE-LOCK-001
2. (지금) 이번 라운드5 수정 4건 + Figma 실사 결과에 대한 사용자 최종 승인 대기
3. `.cursor/plans/PUTDUK_RELEASE_MASTER.plan.md` 정식 생성
4. 기존 21개 plan에 EXECUTION_AUTHORITY=NO 등 frontmatter 반영(REL-017, REL-000/001과 병렬 가능 - 서로 다른 파일이므로 PARALLEL_SAFE)
5. Master Plan Validator 최종 재실행 -> `PLAN_LOCKED = TRUE`
6. `pnpm cursor:sync-plans`
7. REL-000부터 순차 자동 실행(recovery/* 브랜치+PR+merge 모델), REL-701-DB/REL-701 직전에서만 정지(Founder 게이트)
...
```text
TOTAL_REL = 115 (기존 109 + 신규 6: REL-222~224, REL-505~507)
...
`RESPONSIVE_REQUIRED` / `PERFORMANCE_BUDGET_REQUIRED` / `LARGE_SCREEN_REQUIRED`(구조적 안전망 여부만) / `LOW_END_DEVICE_REQUIRED` / `RESPONSIVE_QA` / `PERFORMANCE_QA` - REL-100~130, REL-404에 적용.
