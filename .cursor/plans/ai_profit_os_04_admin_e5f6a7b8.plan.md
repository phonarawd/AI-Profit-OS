...
todos:
  - id: admin-ia-child-routes-911
    ...
    status: completed
  - id: admin-ops
    content: "[grok-4.5|256K] R6 PART0 task-first shell · 12모듈+2b · 첫화면=오늘처리/막힘/위험/최근변경 · 왕초보한글·IT0 · TOP5 · wallet/growth/reports shell · deep≠여기(missions/partners/ticker/coach=전용todo) · verify:admin-routes/admin-plain-language · **v7.24.5 HARDEN:** TOP6 광고성과=Infra `marketing-admin-roas-health` 표시 pointer(retry backlog·permanent fail·last delivery·test/prod) · secret/token 표시 0 · consumer `/ads` `/l/*`를 Admin route로 변경 0 · **v7.24.6 HARDEN:** Founder Operations Dashboard=/admin 흡수(sidebar 13 금지) · 왕초보 4질문 항상 답=지금정상/지금무엇/왜/내할일(기존 오늘처리/막힘/위험/최근변경 매핑) · 최상단=전체시스템·오늘/이달광고비(캡대비)·채널상태·전환추적·SEO·배포·Cursor자동운영·자동처리중N·내할일 · [모든 광고 긴급 정지]≠TOP5 거래회로 · human-readable audit feed(무엇/왜/얼마/정책한도/정상/내할일) · 기술로그=Advanced만 · Human Required Queue=사람만 · 자동처리중=transient 429/retry/CI autofix/SEO PR/CAPI retryable/provider temporary · Cursor run Founder-normalization · Standing Authorization 표시/승인 UX(enforce Owns=06 ads-budget-standing-authorization) · child `/admin/growth?tab=paid-ads`≠`campaigns`(프로모) · 06 runtime 전=빈/준비중 상태(가짜 spend·live pause 0) · TOP5/12모듈/3-mode/Money Confirm 불변 · §9.5.8"
    status: pending
  - id: admin-user-ops
    content: "[grok-4.5|256K] PART1 §37·§39·§9.8.7/8 유저360 + finance KPI + referral/attribution/CS + OAuth Kakao표시 + Passkey + export + RBAC · verify:admin-user-360 · **v7.24.5 HARDEN:** CAPI 진단 Owns=Infra `marketing-admin-roas-health` · Admin은 표시만 · consumer `/ads` `/l/*`를 Admin route로 변경 0 · **v7.24.6 HARDEN:** 유료광고 mutation/Standing Authorization/Emergency stop ≠본 todo · CAPI 표시 only 유지"
    status: pending
  - id: admin-price-sync
  ...
  - id: redesign-r6-admin-three-mode-ops
    content: "[grok-4.5|256K] Redesign R6-3mode · policy/bulk mutation/execution rule/wallet operation/risk threshold에 LIVE|DRY_RUN|SIMULATION 명시 · preview→confirm→apply→result→rollback · mode별 권한/audit/idempotency/impact count · simulation이 live write0 · 쉬운한글 · verify:admin-operation-modes 신설+CATALOG · **v7.24.6 HARDEN:** Standing Authorization 변경·월한도 증가·허용증액범위 확대·광고 긴급정지·새 channel/account/landing/message axis=HIGH+HUMAN · preview→confirm→apply · SIMULATION이 live ads write 0 · 기존 Money/KRW/withdraw Confirm 약화 0"
    status: pending
  ...
  - id: redesign-r6-admin-certification
    content: "[grok-4.5|256K] Redesign R6 certification · 12모듈+2b/child route 전수·task-first·3-mode·RBAC/MFA/IP/audit/rollback·Money/Engine/UI field mapping·Ops Worker/noindex · admin-routes/plain-language/user360/growth/mission/partner/ticker/coach/support/analytics/no-admin-in-web/admin-operation-modes + T0/T1/T2 · known P0~P3 defect0 · **v7.24.6 HARDEN:** Founder 4질문·emergency stop·paid-ads≠campaigns·sidebar13=0·Human Queue vs 자동처리중 분리 · 기존 12모듈+TOP5+3-mode 인증 유지 · **v7.24.7:** 본 todo PASS ≠ 「퍼뜩의 최종 자동운영 출시 준비 완료」 · 실데이터 출시 판정 Owns=06 `ads-autonomous-ops-release-certification` · R6 완료조건 추가 0"
    status: pending
isProject: false
---
...
>
> ```text
> classification = ADMIN_FOUNDER_INTENT_REFERENCE
...
- **v7.24.6:** Founder Operations Dashboard는 `/admin` + `admin-ops`에 흡수한다. sidebar 13 금지. 유료광고 상세=`/admin/growth?tab=paid-ads` ≠ `campaigns`(프로모). 06 Ads runtime 전 빈/준비중 상태만(가짜 spend·live provider pause 0).
...
| `/admin/growth?tab=campaigns` | 11 | 이벤트/캠페인 **(프로모 · 유료광고 아님)** |
| `/admin/growth?tab=paid-ads` | 11 | **v7.24.6** 유료 광고 운영(Orchestrator 상세) · `campaigns`와 혼동 금지 · **sidebar 13 금지** · runtime wire=06 · 06 전=준비중 상태 |
...
| **모든 광고 긴급 정지** (v7.24.6) | ✅ HIGH/HUMAN · 거래회로와 분리 | `admin.ads_spend.emergency_stopped` |
| **Standing Authorization 승인** (v7.24.6) | ✅ HIGH/HUMAN · 3-mode | `admin.ads_standing_authorization.approved` |
...
| `retry_backlog` (v7.24.5 HARDEN) | provider별 PROCESSING+RETRYABLE_FAILED | Worker delivery state · Infra §31.4b |
| `permanent_failed_count` (v7.24.5 HARDEN) | provider별 PERMANENT_FAILED | Worker · secret 0 |
| `last_delivery_at` (v7.24.5 HARDEN) | provider별 최근 DELIVERED/FAILED | Worker |
| `delivery_mode` (v7.24.5 HARDEN) | test \| prod | Worker config · 토큰 값 표시 **0** |

**금지:** `platform_match_rate` 통합 단일 지표 · Worker vs 플랫폼 리포트 **동급 나란히** · **consumer `/ads` `/l/*`를 Admin 화면으로 변경** · access token/secret 표시
...
#### 9.5.8 Founder Operations Dashboard (v7.24.6 · `/admin` 흡수 · sidebar 13 금지)

> **Owns (UX/표시):** `admin-ops` · **enforce/runtime:** Infra §31.9~§31.11 · 06 가산 todo.  
> **선행 화면 계약:** 기존 `오늘처리/막힘/위험/최근변경` + TOP5를 **교체하지 않고** 아래 4질문에 매핑한다.

왕초보 Founder가 기술 지식 없이 항상 답할 수 있어야 한다.

| # | 질문 | 매핑 |
|---|------|------|
| 1 | 지금 정상인가? | 전체 시스템 + 위험 + 채널/추적/SEO/배포/Cursor |
| 2 | 시스템이 지금 무엇을 하고 있는가? | 오늘처리 + `현재 자동 처리 중 N건` |
| 3 | 왜 그렇게 했는가? | 최근변경 + 「방금 시스템이 한 일」 audit feed |
| 4 | 내가 해야 할 일이 있는가? | 막힘 + Human Required Queue |
...
오늘 광고비          ₩… / ₩…(일 캡)
이번 달 광고비       ₩… / ₩…(월 캡)
Meta / TikTok / Google   정상 | 자동 최적화 중 | 준비 중
전환 추적 · SEO · 배포 · Cursor 자동운영
현재 자동 처리 중      N건
내가 해야 할 일       없음 | N건
[모든 광고 긴급 정지]
```

06 runtime 전: 광고비/채널은 **준비 중** · 가짜 ₩143,000 연출 금지 · 긴급정지 버튼은 비활성 또는 「준비 중」(live pause 0).

**방금 시스템이 한 일** — 모든 자동 행동 설명 필수: 무엇을 / 왜 / 얼마가 바뀌었는가 / 어떤 정책·한도 안인지 / 현재 정상인가 / Founder 할 일. 기술 로그=Advanced만.

예: `TikTok 광고 B 일 예산 ₩40,000 → ₩44,000` · 왜=평가 구간 목표 충족 + 자동 조정 허용범위 +15% 이내 · 할 일=없음.  
예: `Meta 광고 C를 자동으로 일시중지` · 왜=전환 측정 불안정 · 할 일=없음.

**Human Required Queue (사람만):** 계정 재연결 · OAuth · 새 channel/account/landing/message axis · Standing Authorization 변경 · 월 한도 증가 · production deploy 승인 · Money/KRW Confirm · 외부 사람 verification.  
**올리지 않음 → 자동 처리 중:** transient 429 · 자동 retry · Cursor가 고칠 수 있는 CI · SEO 자동 수정 PR · CAPI retryable · provider temporary outage.

**Cursor run Founder-normalization:** Internal `Agent FINISHED / PR #n / CI pending` → `수정안을 만들었습니다. 자동 검사를 진행 중입니다.` · `Agent ERROR` → `자동 수정에 실패했습니다. 원인을 다시 확인하고 있습니다.` · 사람 필요 시에만 `직접 확인이 필요합니다.`

**[모든 광고 긴급 정지]** ≠ TOP5 거래 긴급 정지. 광고비 지출 action만. CAPI/analytics 유지. HIGH/HUMAN. wire=06 Orchestrator.

**탭:** `/admin/growth?tab=paid-ads` = 유료 광고 상세 · `/admin/growth?tab=campaigns` = 이벤트/프로모. 혼동 금지.
