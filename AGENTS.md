...
Cursor는 **플랜 집행기**다. ADR 없는 스택 재설계 금지.
이 레포는 **백엔드 전용**이다. 고객 웹은 `phonarawd/putduk-web`.

## 읽기 순서

1. `TOOLCHAIN.md` + `.cursor/rules/*` (always) + 해당 glob rules
2. 착수 전: `docs/CONSTITUTION_BOOTSTRAP.md` (실물·헌법·모델배정 · 이력 스냅샷 포함)
...
5. 도메인 해시 파일만 · **File-Serial:** 한 파일 todos 전부 완료 후 다음 번호 · 한 채팅=한 todo · 접두사 `[grok-4.5|256K]` / `[composer-2.5|200K]`
6. 이 레포 직렬: **01 Money** · **02 Engine** · **02.5 Engine Acceptance QA** · **04 Admin API/RBAC** · **06 Infra**. 03 UI · 05 PWA · launch 플랜은 이 트리에서 제거되었고 고객 웹은 putduk-web.
7. AI 이름=**퍼뜩** (앱명과 동일 · §47 Personal AI · 타프로젝트 코치명 금지)
...
| Cloudflare | Workers / R2 (API·dispatcher · 고객 웹 origin은 putduk-web) |
| Vercel | 영구 금지 |
...
- Platform = AI Profit OS · Legal = `schemas/operator-entity.v1.json`

## 툴체인

- Node22 · pnpm@10.14 · Nest · Rust
- 고객 웹(Next/Tailwind/OpenNext) = putduk-web. 이 레포에 `apps/web` · `apps/admin` · `packages/ui` 재도입 금지.
...
스택 재제안 · Vercel · PG사 SDK · Supabase Auth · 난수 정산 · 잔액 UPDATE · `--no-verify` · `.env` 커밋
